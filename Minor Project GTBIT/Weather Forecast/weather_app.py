...
st.set_page_config(page_title='Udaan : Weather Forecast', page_icon=logo)
...
owm = pyowm.OWM('89645c30a78f585b6ae3a61d491142ce')
mgr = owm.weather_manager()
degree_sign = u'\N{DEGREE SIGN}'

image = Image.open('logoudaanwhite.png')
st.image(image)

st.title("Weather Forecast")

st.write("### Make your journey safe by pre-planning!")
...
def other_weather_updates():
    ...
    if forecaster.will_have_tornado():
        ...
        image = Image.open('tornado.webp')
        st.image(image, use_column_width=True)
    if forecaster.will_have_hurricane():
        st.write("### Hurricane Alert!")
        image = Image.open('hurricane.jpg')
        st.image(image, use_column_width=True)
